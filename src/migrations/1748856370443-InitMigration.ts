...
export class InitMigration1748856370443 implements MigrationInterface {
    name = 'InitMigration1748856370443'
    ...
    public async up(queryRunner: QueryRunner): Promise<void> {
        ...
        await queryRunner.query(`CREATE TABLE \`users\` (\`user_id\` int NOT NULL AUTO_INCREMENT, \`username\` varchar(50) NOT NULL COMMENT '用户名', \`nick_name\` varchar(50) NOT NULL COMMENT '昵称', \`phoneNumber\` varchar(20) NOT NULL COMMENT '手机号', \`password\` varchar(150) NOT NULL COMMENT '密码', \`isFrozen\` tinyint NOT NULL COMMENT '是否冻结' DEFAULT 0, \`isDelete\` tinyint NOT NULL COMMENT '是否删除' DEFAULT 0, \`createTime\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6), \`updateTime\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6) ON UPDATE CURRENT_TIMESTAMP(6), PRIMARY KEY (\`user_id\`)) ENGINE=InnoDB`);
